...
2. Run `mdt dev smoke tool-setups` for the all-tools audit, or `mdt dev smoke tool-setups --tool <claude|cursor|codex>` when you want one tool's local CLI probe.
3. Run `mdt dev smoke workflows --tool <claude|cursor|codex>` for the tool you are validating.
...
- Codex support in this page refers to the current MDT setup built around `.codex/AGENTS.md`, `codex-template/config.toml`, `codex-template/skills/`, and package-selected assets materialized into `~/.codex/`.
