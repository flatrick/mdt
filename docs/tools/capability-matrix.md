...
| Persistent context / memory | `official` `CLAUDE.md` and memory docs | `official` rules, memories, and `AGENTS.md` | `official` persistent layered instructions; `experimental`, `locally-verified` local `memories` feature flag exists; MDT also uses project-local `.codex/` runtime state for explicit continuous-learning | `official` persistent instructions/config; no local verification |
