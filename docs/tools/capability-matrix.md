...
| Repo-defined workflow commands | `official` markdown slash commands | `official`, `locally-verified` custom commands via `cursor-template/commands/*.md` installed to `.cursor/commands/` | `unsupported` as a direct Claude-style markdown command model; Codex docs emphasize built-in slash commands, skills, rules, and AGENTS | `official` `command` entries or markdown command files |
