...
npx MDT-agentshield scan --path ~/.claude/

# Scan with verbose output
npx MDT-agentshield scan --verbose
```
...
Mitigation: pin MCP tool versions, verify tool descriptions haven't changed between sessions, and run `npx MDT-agentshield scan` to detect suspicious MCP configurations.
...
Run `npx MDT-agentshield scan` after each round of fixes to verify your score improves.
...
- [ ] Run `npx MDT-agentshield scan` on your configuration
...
- [ModelDev Toolkit (this fork)](https://github.com/flatrick/modeldev-toolkit) — production-ready agent configurations
