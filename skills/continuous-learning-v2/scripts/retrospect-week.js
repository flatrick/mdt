...
function generateWeeklyRetrospective(options = {}) {
  const weekParts = options.week
    ? parseWeekSpec(options.week)
    : getIsoWeekForDate(options.now || new Date());
  const window = getIsoWeekWindow(weekParts);
  const project = options.project || detectProject(options.cwd || process.cwd());
  ...
  const outputPath = options.outputPath || getWeeklyRetrospectivePath(project, window.week);
