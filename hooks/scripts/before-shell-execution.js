...
readStdin().then(raw => {
  try {
    ...
    if (process.platform !== 'win32' && /(npm run dev\b|pnpm( run)? dev\b|yarn dev\b|bun run dev\b)/.test(cmd)) {
      console.error('[MDT] BLOCKED: Dev server must run in tmux for log access');
      console.error('[MDT] Use: tmux new-session -d -s dev "npm run dev"');
      process.exit(2);
    }
    ...
    if (process.platform !== 'win32' && !process.env.TMUX &&
      ...
      console.error('[MDT] Consider running in tmux for session persistence');
    }
