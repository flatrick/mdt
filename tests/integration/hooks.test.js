...
const { runHookWithInput, runHookCommand } = require('../helpers/hook-integration-test-utils');
...
async function runTests() {
  ...
  if (await asyncTest('hooks.json async hook has valid timeout field', async () => {
    ...
    const isInline = asyncHook.hooks[0].command.startsWith('node -e');
    const isNodeScript = asyncHook.hooks[0].command.startsWith('node "') && /\.js[\s"]/.test(asyncHook.hooks[0].command);
    assert.ok(isInline || isNodeScript, 'Async hook command should be node -e or node "script.js" format');
  })) passed++; else failed++;
    ...
    for (const [hookType, hookArray] of Object.entries(hooks.hooks)) {
      for (const hookDef of hookArray) {
        ...
        for (const hook of hookDef.hooks) {
          ...
          const isNodeScript = isFilePath && /\.js[\s"]/.test(hook.command);

          assert.ok(
            isInline || isNodeScript,
            `Hook command in ${hookType} must be inline (node -e) or node " ... .js (Node-only; no .sh/.ps1), got: ${hook.command.substring(0, 80)}`
          );
        }
      }
    }
